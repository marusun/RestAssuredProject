...
public class UserTests2 {
	...
	public void testGetUserByName() {
		...
		Response response = UserEndPointsWithPropertyFile.readUser(userPayload.getUsername());
		response.then().log().all();
	...
	public void testUpdateUserByName() {
		...
		userPayload.setLastName(fake.name().lastName());
		userPayload.setEmail(fake.internet().emailAddress());
		Response response = UserEndPointsWithPropertyFile.updateUser(userPayload.getUsername(),userPayload);
		response.then().log().all();

		AssertJUnit.assertEquals(response.getStatusCode(), 200);
		logger.info("---------------updation of User done------------");
		Response responseAfterUpdate = UserEndPointsWithPropertyFile.readUser(userPayload.getUsername());
		response.then().log().all();
	...
	public void testDeleteUserByName() {
		...
		Response response = UserEndPointsWithPropertyFile.deleteUser(userPayload.getUsername());
		response.then().log().all();
